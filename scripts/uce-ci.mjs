...
function toSarif(value, findings) {
  ...
  return { $schema: 'https://json.schemastore.org/sarif-2.1.0.json', version: '2.1.0', runs: [{ tool: { driver: { name: 'Ryzova UCE CI', version: value.analysisVersion, informationUri: 'https://uce.ryzova.com/', rules } }, results: findings.map((item) => ({ ruleId: item.ruleId, level: item.severity === 'critical' ? 'error' : item.severity === 'warning' ? 'warning' : 'note', message: { text: `${item.message} Recommendation: ${item.recommendation}` }, locations: item.file ? [{ physicalLocation: { artifactLocation: { uri: String(item.file).replace(/\\/g, '/') }, region: item.line ? { startLine: item.line } : undefined } }] : [], properties: { module: item.module } })) }] };
}
