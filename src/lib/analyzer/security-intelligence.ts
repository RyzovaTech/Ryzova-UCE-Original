...
const RULES: Rule[] = [
  ...
  { id: 'SEC005', title: 'Insecure HTTP endpoint', severity: 'warning', pattern: /https?:\/\//i, evidence: 'A URL literal was found; HTTP URLs require review when used for sensitive traffic.', recommendation: 'Use HTTPS for production network traffic and avoid sending secrets over HTTP.' },
...
export function detectSecurityIntelligence(files: ProjectFile[]): SecurityIntelligence {
  ...
  const sourceFiles = files.filter((file) => !file.isDirectory && SOURCE_RE.test(file.path));
  for (const file of sourceFiles) {
    ...
    for (const rule of RULES) {
      ...
      let emitted = 0;
      while ((match = rule.pattern.exec(source)) && emitted < 3) {
        const line = source.slice(0, match.index).split('\n').length;
        findings.push({ id: `${rule.id}-${file.path}-${line}`, title: rule.title, severity: rule.severity, file: file.path, line, evidence: rule.evidence, recommendation: rule.recommendation });
        emitted += 1;
      }
    }
  }
