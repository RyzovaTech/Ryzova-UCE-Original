  ...
  return (
    <Card className="sticky top-20">
      <CardHeader>
        ...
        <CardTitle className="pt-2 text-xl">{item.name}</CardTitle>
...
export function CatalogPage() {
  ...
  const [section, setSection] = useState<'all' | CatalogSection>('all'); const [selectedId, setSelectedId] = useState(UCE_CATALOG_ITEMS[0].id);
  ...
  const visible = filtered.slice(0, RESULT_LIMIT); const selected = visible.find((item) => item.id === selectedId) ?? visible[0];
  return (
    <div className="space-y-6">
      <section className="rounded-xl border bg-gradient-to-br from-primary/10 via-card to-card p-6">
        <div className="flex items-start gap-4"><div className="rounded-lg bg-primary/10 p-3 text-primary"><BookOpen className="h-6 w-6" /></div><div><h1 className="text-2xl font-semibold tracking-tight">UCE Catalog</h1><p className="mt-2 max-w-3xl text-sm leading-6 text-muted-foreground">Explore the languages, technologies, compatibility rules, and intelligence capabilities understood by Ryzova UCE™. Catalog entries describe deterministic knowledge—not a guarantee that every project has enough evidence for detection.</p></div></div>
      </section>
      ...
      <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_360px]">
        <section aria-live="polite"><div className="mb-3 flex items-center justify-between"><h2 className="font-semibold">{filtered.length.toLocaleString()} matching entries</h2>{filtered.length > RESULT_LIMIT ? <span className="text-xs text-muted-foreground">Showing first {RESULT_LIMIT}</span> : null}</div>
          <div className="grid gap-3 sm:grid-cols-2">{visible.map((item) => <button key={item.id} type="button" onClick={() => setSelectedId(item.id)} className={`rounded-lg border p-4 text-left transition-colors hover:border-primary/50 hover:bg-muted/40 ${selected?.id === item.id ? 'border-primary bg-primary/5' : 'bg-card'}`}><div className="flex items-start justify-between gap-3"><h3 className="font-medium">{item.name}</h3><Badge variant="secondary" className="shrink-0 text-[10px]">{item.category}</Badge></div><p className="mt-2 line-clamp-2 text-sm leading-5 text-muted-foreground">{item.description}</p></button>)}</div>
        ...
        <aside aria-label="Catalog entry details">{selected ? <CatalogDetail item={selected} /> : null}</aside>
      </div>
    </div>
  );
}
